...
function SectionHeader({ label, sub, accent, count }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, gridColumn: '1/-1' }}>
...
export default function GridView({ tasks, onDelete, onTap, groupBy }) {
  let sections = []
      ...
      {sections.map(sec => (
        <div key={sec.key} style={{ marginBottom: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
            ...
            <AnimatePresence>
              {sec.tasks.map(task => (
                <TaskCard key={task.id} task={task} variant="grid" onDelete={onDelete} onTap={onTap} />
              ))}
            </AnimatePresence>
          </div>
          {sec.tasks.length === 0 && (
